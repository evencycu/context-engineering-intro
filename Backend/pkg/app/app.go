...
func NewApplication(cfg *configs.Config, logger *logrus.Logger) (*Application, error) {
	...
	// Initialize notification processor (unified WorkerPool + QueueConsumer + QueueProducer)
	notificationProcessor := actor.NewNotificationProcessor(
		redisClient,
		actor.NewWorkerDB(notificationDestRepo, installationRepo, teamsBotRepo, notificationRepo, projectRepo),
