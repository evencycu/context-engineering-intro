...
import { MessagePriority, Template } from '../types';
...
export const Compose: React.FC = () => {
  ...
  useEffect(() => {
    ...
    if (!currentProject || !currentProject.id) {
      console.log('[Compose] No current project yet, waiting...');
      return;
    }
    ...
    if (!isValidUUID) {
      console.warn(`[Compose] Invalid project ID format: ${currentProject.id}. Expected UUID. Skipping template load.`);
      setProjectTemplates([]);
      setIsLoadingTemplates(false);
      return;
    }
    
    console.log(`[Compose] Project changed to: ${currentProject.name} (${currentProject.id}), loading templates...`);
    loadTemplates();
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentProject.id]); // Only depend on project ID to avoid unnecessary re-renders
  ...
  const loadTemplates = async () => {
    if (!currentProject || !currentProject.id) {
      console.warn('[Compose] No current project, skipping template load');
      return;
    }
    ...
    console.log(`[Compose] Loading templates for project: ${currentProject.name} (${currentProject.id})`);
    setIsLoadingTemplates(true);
    try {
      // Load project templates using project UUID
      const projectTpls = await service.getTemplates(currentProject.id);
      console.log(`[Compose] Loaded ${projectTpls.length} project templates`);
      
      // Try to load global templates, but don't fail if it doesn't work
      let globalTpls: Template[] = [];
      try {
        globalTpls = await service.getGlobalTemplates();
        console.log(`[Compose] Loaded ${globalTpls.length} global templates`);
      } catch (globalError: any) {
        // Global templates API might not be available, log but don't fail
        console.warn('[Compose] Failed to load global templates (this is OK if API is not available):', globalError?.message);
      }
      
      // Combine project templates and global templates
      setProjectTemplates([...projectTpls, ...globalTpls]);
      console.log(`[Compose] Total templates available: ${projectTpls.length + globalTpls.length}`);
    } catch (e: any) {
      console.error('[Compose] Failed to load project templates:', e);
      console.error('[Compose] Error details:', {
        message: e?.message,
        stack: e?.stack,
            ...
            <div className="bg-gray-50 border-b border-gray-200 px-6 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex bg-white rounded-lg p-1 border border-gray-200 shadow-sm w-fit">
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${mode === 'template' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:text-gray-900'
                    }`}
                >
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-4 py-2 rounded-md text-sm font-medium transition-colors ${mode === 'json' ? 'bg-blue-50 text-blue-700' : 'text-gray-600 hover:text-gray-900'
                    }`}
                >
                  <Code size={16} />
                  JSON
              ...
              <div className="flex gap-2">
                {(['User', 'List', 'Tag'] as const).map((t) => (
                  <button
                    ...
                    className={`flex-1 py-1.5 text-xs font-medium rounded border ${targetType === t
